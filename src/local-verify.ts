...
async function localVerify() {
  try {
    logger.info('Verifying local environment...', { cwd: process.cwd() });
    ...
    logger.info('Local verification completed', { success: true });
  } catch (error) {
    logger.error('Error verifying local environment:', { error });
    process.exit(1);
  }
}
