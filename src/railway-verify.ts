...
async function railwayVerify() {
  try {
    logger.info('Verifying Railway deployment...', { cwd: process.cwd() });
    ...
    logger.info('Railway verification completed', { success: true });
  } catch (error) {
    logger.error('Error verifying Railway deployment:', { error });
    process.exit(1);
  }
}
